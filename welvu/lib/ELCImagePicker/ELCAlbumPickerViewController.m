...
- (void)viewDidLoad {
   ...
   // albumTableView.backgroundView = [[UIImageView alloc]initWithImage:[UIImage imageWithContentsOfFile:ALBUM_IMAGE_PNG]];
...
-(void)reloadTableView {
    ...
    if (myInt >= 7.0 && myInt < 8.0) {
        ...
        [titleLabel release];
         albumTableView.frame = CGRectMake(0, 0, 320, 670);
    }else if (myInt >= 8.0 ) {
        self.title = NSLocalizedString(@"Select an Album", @"");
        UILabel *titleLabel = [[UILabel alloc] initWithFrame:CGRectZero];
        titleLabel.text = self.title;
        ...
        self.navigationItem.titleView = titleLabel;
        [titleLabel release];
        albumTableView.frame = CGRectMake(0, 40, 320, 630);
